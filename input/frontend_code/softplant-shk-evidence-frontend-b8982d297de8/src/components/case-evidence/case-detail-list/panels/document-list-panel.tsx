...
export default function DocumentListPanel({
  ...
  const isOcrReady = useCallback((row: any) => {
    const ocrStatus = String(row?.ocr_status ?? row?.ocrStatus ?? '')
      .toUpperCase()
      .trim();
    // OCR 완료 기준: ocr_status === 'COMPLETED'
  ...
  const appliedPowerSearch = String(civilListQuery?.power_search ?? '').trim();
  const appliedSearchLabel = appliedPowerSearch ? appliedPowerSearch.replace(/#/g, '') : appliedKeyword;
  const isSearchMode = isCivilMode && (!!appliedKeyword || !!appliedPowerSearch);

  const extractSearchTerms = (searchStr: string) => {
    if (!searchStr) return '';
    // AND, OR, (, ) 앞에 #을 붙임 (EvidenceTable과 동일)
    return searchStr
  ...
  const HeaderResizeHandle = ({ col }: { col: TCivilColKey }) => (
    ...
    >
      {/* hover indicator: sky-blue vertical line only */}
  ...
  const openHeaderMenu = (key: TCivilHeaderKey, e: React.MouseEvent) => {
    ...
    // 작성자: root 메뉴(정렬/필터) + 정렬은 hover flyout
    // 태그: 필터만
    // 나머지: 정렬만
  ...
  const HeaderSortIcon = ({ active }: { active: boolean }) => {
    ...
    return (
      <span className='ml-2 inline-flex items-center'>
        {/* 아이콘만 있는 헤더(예: 북마크)에서 시각적 압축 방지를 위해 18px 고정 */}
  ...
  const openEvidenceLikeViewer = (row: any, kind: 'pdf' | 'text') => {
    ...
    // 민사 문서 뷰어(새탭)는 CaseViewerPage의 /case-document/document API 사용
    ...
    // split/evidence id가 없는 경우 file_url로 직접 열기
  ...
  const formatKoreanDate = (raw: unknown) => {
    ...
    // 신규 백엔드 형식: YYYYMMDD (예: "20200717") → "2020.07.17"
    ...
    // YYYY.MM.DD 또는 YYYY-MM-DD 형식이면 YYYY.MM.DD로 정규화
  ...
  // "+ 태그"는 행 hover가 아니라, 태그 버튼 hover일 때만 노출
  const [hoveredTagRowId, setHoveredTagRowId] = useState<string | null>(null);
  const toggleSelectedRowId = (id: string) =>
  ...
  // Panel-width 기반으로 필터 UI를 전환한다.
  // - 최소폭으로 줄였을 때(필터 라벨이 잘리는 폭) 무조건 Select로 전환.
  ...
  const civilSearchWidth = useMemo(() => {
    // 패널이 좁을 때 검색 input을 162px로 축소 (기본 240px)
  ...
  // Drag & Drop UI 상태: civilRows를 기반으로 한 로컬 렌더 순서(optimistic reorder)
  ...
  const runPinToggle = async (row: any) => {
    ...
    // UX: 핀 고정/해제 완료 후에는 체크박스 선택을 해제한다.
    setSelectedRowIds([]);
  ...
  return (
    <div className='flex min-h-0 w-full flex-1 flex-col'>
          ...
          <div className='flex items-start gap-2 pb-[12px] pl-[15px] pt-[12px]'>
            ...
            <div className='flex flex-col'>
              <div className='relative' style={{ width: civilSearchWidth }}>
                ...
                {/* clear (X) when applied keyword/power_search exists */}
                {isSearchMode ? (
                  <button
                    type='button'
                    ...
                    onClick={() => {
                      ...
                      setResetPowerSearch(true);
                      setTimeout(() => setResetPowerSearch(false), 0);
                      onChangeCivilListQuery?.({
                        keyword: '',
                        power_search: '',
                        keywordVersion: Date.now(),
              ...
              {/* result line + reset (same UX as EvidenceTable) */}
              {isSearchMode ? (
            ...
            <button
              type='button'
              className='flex h-[32px] w-[92px] items-center justify-center gap-2 rounded-[10px] bg-[#F4F4F5] text-[14px] font-semibold text-[#18181B] hover:bg-[#E4E4E7]'
              onClick={() => {
                if (ocrReadyDocCount === 0) {
                  toastOcrNotReady();
                  return;
            ...
            >
              <UserSearch className='h-4 w-4 text-[#18181B]' />
              파워검색
            </button>
          </div>
        ) : (
                  ...
                  >
                    <table ref={civilTableRef} className='w-full min-w-full table-fixed'>
                      ...
                      <colgroup>
                        ...
                        <col style={{ width: columnWidths.category, minWidth: columnWidths.category }} />
                        <col style={{ width: columnWidths.docName, minWidth: columnWidths.docName }} />
                        <col style={{ width: columnWidths.submitter, minWidth: columnWidths.submitter }} />
                      ...
                      <thead className='sticky top-0 z-10 bg-white'>
                        <tr className='text-left text-[12px] font-semibold text-[#8A8A8E]'>
                          ...
                          <HeaderCell label='구분' hkey='category' enableSort={true} sortCol='parsed_category' />
                          <th
                          ...
                          <HeaderCell
                            ...
                            enableSort={true}
                            enableFilter={true}
                            sortCol='parsed_submitter_name'
                          />
                          <HeaderCell label='태그' hkey='tags' enableFilter={true} />
                      ...
                      <Droppable
                        ...
                        // 드래그 중인 <tr>은 <table> 밖으로 portal되어 <colgroup> 너비를 잃음
                        // 동일한 <colgroup>을 가진 실제 <table> 내부에 클론을 렌더링해 미리보기와 일치시킴
                        renderClone={(cloneProvided, _cloneSnapshot, rubric) => {
                          // getBoundingClientRect().width: 가시 너비(수평 스크롤 시 실제보다 작을 수 있음)
                          // scrollWidth: 전체 렌더링 너비(드래그 미리보기 1:1 일치에 사용)
                          ...
                          const category = String((displayRow as any)?.parsed_category ?? (displayRow as any)?.parsedCategory ?? '');
                          ...
                          const submitterName = String(
                            (displayRow as any)?.parsed_submitter_name ?? (displayRow as any)?.parsedSubmitterName ?? '',
                          );
                          ...
                          const tagsRaw = Array.isArray((displayRow as any)?.tags) ? (displayRow as any).tags : [];
                          const tagsNormalized = tagsRaw
                            .map((t: any) => {
                              if (!t) return null;
                            ...
                            <table className='table-fixed' style={{ width: tableWidth, minWidth: tableWidth }}>
                              ...
                              <tbody>
                                ...
                                >
                                  ...
                                  <td
                                    className='border-b border-[#E4E4E7] bg-white px-3 text-[14px] font-medium'
                                  ...
                                  <td className='border-b border-[#E4E4E7] bg-white px-3 text-[13px] font-normal text-[#000000]'>
                                    {submitterName || '-'}
                                  </td>
                                  <td className='border-b border-[#E4E4E7] bg-white px-3 py-2'>
                                    {tagLabelItems.length ? (
                                      <div className='flex w-full flex-wrap items-center gap-1 text-[12px] font-semibold text-[#000000]'>
                                      ...
                                      <div className='flex h-[24px] items-center rounded-[8px] border border-[#E4E4E7] bg-transparent px-2 text-[12px] font-medium text-[#000000]'>
                                        태그
                                      </div>
                                    )}
                                  </td>
                        ...
                        {(dropProvided) => (
                          <tbody ref={dropProvided.innerRef} {...dropProvided.droppableProps}>
                            {visibleCivilTableRows.length === 0 ? (
                              <tr>
                                <td colSpan={11} className='px-6 py-12 text-center text-[14px] text-[#8A8A8E]'>
                              ...
                              visibleCivilTableRows.map((row: any, idx: number) => {
                                ...
                                const dateRaw =
                                  ...
                                  '';
                                const date = formatKoreanDate(dateRaw);
                                const category = String(displayRow?.parsed_category ?? displayRow?.parsedCategory ?? '');
                                const docName = String(
                                  displayRow?.parsed_sub_category ?? displayRow?.parsedSubCategory ?? displayRow?.title ?? '',
                                );
                                const submitterName = String(displayRow?.parsed_submitter_name ?? displayRow?.parsedSubmitterName ?? '');
                                ...
                                const tagsRaw = Array.isArray(displayRow?.tags) ? displayRow.tags : [];
                                const tagsNormalized = tagsRaw
                                  .map((t: any) => {
                                    if (!t) return null;
                                    if (typeof t === 'string') return { tag_set_id: '', tag_name: t, color: '', tag_id: '' };
                                  ...
                                  .filter((t: any) => t && String(t.tag_name ?? '').trim());
                                const hasTags = tagsNormalized.length > 0;
                                const tagLabelItems = tagsNormalized.map((t: any) => ({
                                  tag_set_id: String(t.tag_set_id ?? ''),
                                  tag_name: String(t.tag_name ?? '').trim(),
                                  color: String(t.color ?? ''),
                                ...
                                return (
                                  <Fragment key={id || `${idx}:${docName}`}>
                                    <Draggable draggableId={id || `row-${idx}`} index={idx} isDragDisabled={!id}>
                                      {(dragProvided) => (
                                        ...
                                        >
                                          ...
                                          {/* 구분: 제출자가 원고대리인일 때만 빨강(#B91C1C) / 14 / medium */}
                                          <td
                                            className={`px-3 text-[14px] font-medium ${rowBgClass} ${rowCellBorderClass}`}
                                            style={{ color: categoryAndNameColor }}
                                          >
                                            {category || '-'}
                                          </td>
                                          ...
                                          {/* 작성자: parsed_submitter_name */}
                                          <td className={`px-3 text-[13px] font-normal text-[#000000] ${rowBgClass} ${rowCellBorderClass}`}>
                                            {submitterName || '-'}
                                          </td>
                                          <td className={`px-3 py-2 ${rowBgClass} ${rowCellBorderClass}`}>
                                            {/* 태그: 여러 개일 수 있음 -> 라벨을 여러 개 표시 */}
                                            {hasTags ? (
                                              <button
                                                type='button'
                                                ...
                                                onClick={(e) => {
                                                  ...
                                                  setTagPopup({
                                                    ...
                                                    left,
                                                    rowId: id,
                                                    civilCaseId: getRowCivilCaseId(displayRow),
                                                    projectId: String(
                                              ...
                                              >
                                                {isTagHovered ? <Plus className='h-3 w-3 shrink-0' /> : null}
                                                <span>{isTagHovered ? '태그' : '태그'}</span>
                                              </button>
                                            )}
                                          </td>
                                          <td
                                    ...
                                    {hasMatchHighlights(displayRow) ? (
                                      <tr>
                                        {/* highlight row should start from "구분" column (skip drag/check/pin) */}
                                        <td className={`px-0 ${rowBgClass}`} />
                                        <td className={`px-0 ${rowBgClass}`} />
                                        <td className={`px-0 ${rowBgClass}`} />
                                        <td colSpan={8} className={`p-2 text-[12px] text-[#888] ${rowBgClass}`}>
                                          <div
                                            style={{
                                              display: '-webkit-box',
      ...
      {/* Civil table header sort/filter menu */}
            ...
            <>
              ...
              {(() => {
                ...
                return (
                  ...
                  >
                    {/* reuse sort renderer for submitter */}
                    <div className='py-1'>
                      <button
                        type='button'
      ...
      {/* Civil record list: Power Search modal (same as EvidenceTable) */}
      {isCivilMode ? (
        <PowerSearchModal
          isOpen={isPowerSearchModalOpen}
          onClose={() => setIsPowerSearchModalOpen(false)}
          reset={resetPowerSearch}
          onSearch={(powerSearchRaw) => {
            const processed = extractSearchTerms(String(powerSearchRaw ?? '').trim());
            setRecordSearchQuery('');
            onChangeCivilListQuery?.({
              keyword: '',
              power_search: processed,
              keywordVersion: Date.now(),
              page: 1,
            });
          }}
