...
import { fetchCreateDocument } from '@/apis/case-api/civil-case-api';
...
interface IEvidenceListUploadModalProps {
  ...
  /** 민사 케이스 문서 업로드용 */
  civilCaseId?: string;
  /** (호환) 기존 prop명이 projectId로 되어있던 경우가 있어 fallback으로 사용 */
  projectId?: string;
  /** 원고/피고 여부 (미지정 시 true) */
  isPlaintiff?: boolean;
  /** (선택) 문서 타입: WRITTEN(준비서면) | EVIDENCE(증거) | OTHER(기타) - 기본값: OTHER */
  documentType?: 'WRITTEN' | 'EVIDENCE' | 'OTHER';
  /** (선택) 문서 source_type: LAWYER | CLIENT */
...
export const CaseUploadModal = ({
  ...
  // 허용: PDF, Word, 이미지 / 제외: 한글(HWP/HWPX)
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    for (const file of filesArray) {
      ...
      if (fileExtension && blockedExtensions.includes(fileExtension)) {
        onMessageToast({
          message: `${file.name}은(는) 한글(HWP/HWPX) 파일 업로드를 지원하지 않습니다.`,
      ...
      if (!fileExtension || !allowedExtensions.includes(fileExtension)) {
        onMessageToast({
          message: `${file.name}은(는) 지원하지 않는 파일 형식입니다. (PDF/DOC/DOCX/이미지)`,
          icon: <FaExclamationCircle className='h-5 w-5 text-yellow-500' />,
        });
        continue;
      }
  ...
  // ! 업로드 오류 메시지를 사용자 친화적 텍스트로 변환
  const toUserFriendlyUploadErrorMessage = (err: unknown): string => {
    const stripTrailingFilename = (msg: string): string => {
      const s = String(msg ?? '').trim();
      if (!s) return '';

      // 지원 확장자(및 pdf)로 끝나는 파일명이 콜론 뒤에 붙는 케이스 제거
      // 예: "...없습니다: something.pdf" -> "...없습니다"
      const m = s.match(/^(.*?)(?::\s*[^:]*\.(pdf|docx?|png|jpe?g|gif|bmp|tiff?|webp|heic))\s*$/i);
      if (m?.[1]) return m[1].trim();

      // 콜론이 없더라도 trailing "xxx.ext" 형태면 제거
      const stripped = s.replace(/\s*\S+\.(pdf|docx?|png|jpe?g|gif|bmp|tiff?|webp|heic)\s*$/i, '').trim();
      return stripped || s;
    };

    // 서버가 내려주는 message를 우선 사용
    if (isAxiosError(err)) {
      const data: any = err.response?.data;
      const serverMsg = typeof data?.message === 'string' ? data.message.trim() : '';
      if (serverMsg) {
        return stripTrailingFilename(serverMsg);
      }

      // HTTP 상태 코드 기반 메시지는 사용자에게 노출하지 않고 한국어로 치환
      ...
      if (status && status >= 500) return '서버 오류로 업로드에 실패했습니다. 잠시 후 다시 시도해주세요.';
    }

    // (호환) success:false로 내려온 message가 코드 형태면 사용자 메시지로 치환
    const raw = typeof (err as any)?.message === 'string' ? String((err as any).message).trim() : '';
    const cleaned = stripTrailingFilename(raw);
    if (cleaned && cleaned !== raw) return cleaned;
    if (raw && /^[A-Z0-9_:-]+$/.test(raw)) return '업로드에 실패했습니다. 잠시 후 다시 시도해주세요.';
    if (raw) return raw;
  ...
  const onSubmit = async (data: TForm) => {
    ...
    try {
      ...
      const caseId = civilCaseId ?? projectId ?? '';
      ...
      if (!caseId) {
        onMessageToast({
          message: 'civil_case_id가 없어 업로드할 수 없습니다.',
          icon: <FaExclamationCircle className='h-5 w-5 text-yellow-500' />,
        });
        setIsSubmitting(false);
        return;
      ...
      for (const file of filesToUpload) {
        try {
          ...
          const uploadResponse = await fetchCreateDocument({
          ...
          if (uploadResponse.success) {
            ...
            const errorMsg = toUserFriendlyUploadErrorMessage({ message: uploadResponse.message || '' });
          ...
          newFailedFiles.push(file);
          newFailedErrors[file.name] = toUserFriendlyUploadErrorMessage(error);
  ...
  return (
    <div className={`fixed inset-0 z-[99] flex items-center justify-center bg-[#999] bg-opacity-80 ${isOpen ? '' : 'hidden'}`}>
      <div className='relative max-h-[90vh] overflow-y-auto rounded-[16px] bg-[#fff] p-[32px] lg:max-w-[500px]'>
        <div className='text-[24px] font-bold'>기록 추가</div>
        <div className=''>
          <div className='pt-[24px]'>
            ...
            {/* <p className={`text-[#1890FF] ${getFontSizeClass(12, fontSizeAdjustment)}`} style={{ fontSize: `${getAdjustedSize(12)}px` }}>
              PDF/DOC/DOCX/이미지 파일을 업로드해주세요. (HWP/HWPX는 지원하지 않습니다)
            </p> */}
          </div>
          <div className=''>
            <div
