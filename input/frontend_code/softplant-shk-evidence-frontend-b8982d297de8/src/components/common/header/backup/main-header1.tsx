...
export const MainHeader = () => {
  ...
  const handleSave = async () => {
    ...
    onMessageToast({
      message: result?.isSuccess ? '사건명이 수정되었습니다.' : '사건명 수정에 실패했습니다.',
  ...
  return (
    <div className='fixed top-[52px] z-50 w-full'>
        ...
        <div id='evidence-table-body' className='flex min-w-[90%] items-center 2xl:min-w-[90%]'>
                ...
                {isEditing ? (
                  <div className='flex items-center'>
                    <input
                      ...
                      className='mr-2 min-w-[850px] max-w-[850px] overflow-hidden text-ellipsis whitespace-nowrap rounded border border-gray-300 px-2 py-1 text-[28px] font-bold focus:border-blue-500 focus:outline-none'
                      autoFocus
                      maxLength={50}
                    />
                    <button
                      onClick={handleSave}
                      className='mr-2 h-[32px] w-[50px] rounded bg-[#0050B3] text-sm text-white hover:bg-blue-600'
                    >
                      {isPending ? '저장중...' : '저장'}
                    </button>
                    <button onClick={handleCancel} className='h-[32px] w-[50px] rounded border border-gray-300 text-sm hover:bg-gray-100'>
                      취소
                    </button>
                  </div>
                ) : (
                  <>
                    <div className='flex items-center'>
                      <div className='mr-2 max-w-[850px] overflow-hidden truncate whitespace-nowrap'>{projectName}</div>
          ...
          <div className='flex w-full items-center justify-end'>
            ...
            <span className='ml-1 cursor-pointer'>
              ...
              {/* 설정 드롭다운 메뉴 추가 */}
              {isSettingDropdownOpen && (
                <div ref={dropdownRef} className='absolute top-14 z-10 w-[132px] rounded-[8px] border border-[#E5E5E5] bg-white shadow-lg'>
                  <div className='flex flex-col'>
                    ...
                    >
                      <FiUser className='mr-2 text-xl text-[#888]' />
                      설정
                    </button>
                    <button
                      className='flex h-[44px] w-full items-center border-b border-[#E5E5E5] pl-[16px] text-[14px] text-[#252525] hover:bg-[#F5F5F5]'
                      onClick={() => {
                        /*       navigate('/payment'); */
                        onMessageToast({
                    ...
                    >
                      <PiMonitorLight className='mr-2 text-xl text-[#252525]' />
                      결제관리
                    </button>
                    <button
                      className='flex h-[44px] w-full items-center pl-[16px] text-[14px] text-[#252525] hover:bg-[#F5F5F5]'
