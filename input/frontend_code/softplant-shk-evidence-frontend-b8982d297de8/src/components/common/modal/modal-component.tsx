...
const ModalComponent = ({ header, sendMessage, storageMessage, setIsModalOpen: setIsModalOpen }: IModalProps) => {
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center'>
      ...
      <div className='z-10 w-[80%] rounded-lg bg-[#EFFBFF] p-4 shadow-xl lg:h-auto lg:w-[30%]'>
        ...
        <div className='mb-2 mt-2 flex justify-center' onClick={() => setIsModalOpen()}>
          <button className='w-full rounded-lg bg-[#87CEEB] py-2 text-white hover:bg-sky-700'>확인</button>
        </div>
      </div>
    </div>
  );
