...
import { useFindUserInfo, useExcludeProject, useFindProjectMembers } from '@/hooks/react-query';
...
export const PaymentParticipationModal = ({
  ...
  console.log('PaymentParticipationModal 렌더링:', { isOpen, newBillingKey, newCustomerKey, projectName, projectId, userId });
  ...
  }, [isOpen, projectMembers, findEvidenceUserInfo?.data?.user_id]);
  ...
  const calculatePaymentAmounts = useCallback(() => {
    ...
    // 사건 관리자 결제 (고정 19,000원)
    payments.push({
      type: 'case_subscription',
      amount: 19000,
      memberId: 'manager',
      memberName: '사건관리자',
    });
  ...
  }, [projectMembers, memberPaymentMethods, findEvidenceUserInfo?.data?.user_id]);

  // 사건 나가기 모달 열기
  const handleRemoveMember = (member: { user_id: string; name: string }) => {
    setMemberToRemove({ user_id: member.user_id, name: member.name });
  ...
  const handleTossPayment = useCallback(async () => {
    ...
    if (hasExistingBillingKey && _existingBillingKey) {
      ...
      try {
        const { payments } = calculatePaymentAmounts();
        ...
        const getPlanId = (paymentType: string) => {
          if (paymentType === 'case_subscription') {
            return 'plan_01K5B9Y83AKY5XX8Z78D7HSC73'; // 사건 관리자 결제 (19,000원)
          } else if (paymentType === 'case_participation') {
        ...
        for (const payment of payments) {
          ...
          // 결제 유형에 따라 user_id 설정
          const getUserId = (paymentType: string, memberId: string) => {
            if (paymentType === 'case_subscription') {
              return findEvidenceUserInfo?.data?.user_id || '';
            } else if (paymentType === 'case_participation') {
              return memberId;
            }
            return findEvidenceUserInfo?.data?.user_id || '';
          ...
          const paymentData = {
            user_id: getUserId(payment.type, payment.memberId),
      ...
      >
        ...
        <div className='w-full pt-[32px]'>
          ...
          <div className='mb-[20px] pt-[8px]'>
            ...
            <div className='flex items-center justify-between'>
              ...
              <div className='text-right'>
                <p className='text-[16px] font-bold text-[#5B5B5B]'>₩19,000</p>
              </div>
            </div>
                ...
                <div className='mb-[20px]'>
                  ...
                  <div className='space-y-[12px]'>
                    {filteredMembers.map((member) => (
                      <div key={member.user_id} className='items-center justify-between rounded-[8px] bg-[#F7F8F8] px-[16px] py-[12px]'>
                        <div className='flex items-center'>
                          <div className='flex items-center space-x-3'>
                            ...
                            <span className='text-[14px] font-medium text-[#5B5B5B]'>{member.name}</span>
                          </div>
                          <button
                            onClick={() => handleRemoveMember(member)}
                            className='ml-2 flex h-6 w-6 items-center justify-center rounded-full hover:bg-red-100'
                            title='사건 나가기'
                          >
                            <IoIosCloseCircle className='text-[18px] text-[#8E8E8E] hover:text-red-700' />
                          </button>
                        </div>
                        ...
                        <div className='mt-[12px] flex items-center justify-between space-x-4'>
                          <div className='flex items-center space-x-4'>
                            <label className='flex items-center space-x-2'>
                              <input
                                type='radio'
                                name={`payment_${member.user_id}`}
                                value='me'
                                checked={memberPaymentMethods[member.user_id] === 'me'}
                                onChange={() =>
                            ...
                            <label className='flex items-center space-x-2'>
                              <input
                                ...
                                name={`payment_${member.user_id}`}
                                value='participant'
                                checked={memberPaymentMethods[member.user_id] === 'participant'}
                                onChange={() =>
                                  setMemberPaymentMethods((prev) => ({
                                    ...prev,
                                    [member.user_id]: 'participant',
                                  }))
                                }
                                className='text-[#004AA4]'
                              />
                          ...
                          <span className='text-[16px] text-[#5B5B5B]'>
                            {memberPaymentMethods[member.user_id] === 'me' ? '19,000원' : '0원'}
                          </span>
                        </div>

                        {/* 결제자 표시 */}
                        {memberPaymentMethods[member.user_id] === 'participant' && (
                          <div className='mt-[10px] text-[14px] text-[#1890FF]'>{member.name}님이 결제해요.</div>
                        )}
                      </div>
                    ))}
                  </div>
          ...
          <div className='mt-[20px] flex w-full gap-3'>
            ...
            >
              {isLoading ? '결제 진행중...' : hasExistingBillingKey ? '결제하기' : '카드 등록 및 결제하기'}
            </button>
            <button
      ...
      {isRemoveMemberModalOpen && memberToRemove && (
        <div className='fixed inset-0 z-[1000] flex items-center justify-center bg-black bg-opacity-50'>
          ...
          >
            {/* 제목 */}
            <div className='text-center'>
              <h3 className='text-[18px] font-extrabold text-[#252525]'>사건나가기 처리하시겠어요?</h3>
            </div>

            {/* 설명 */}
            <div className='mb-[24px] mt-[16px] text-center'>
              <div className='text-[14px] leading-[20px] text-[#666666]'>
                결제 목록에서 제거하면 해당 사용자는
                <br />
                더이상 사건에 접근할 수 없습니다.
              </div>
            </div>
