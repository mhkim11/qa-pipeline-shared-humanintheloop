...
import { fetchListHistoryActionFilter } from '@/apis'; // API import
...
import { useHistoryFiltersEvidence, useFindHistoryList } from '@/hooks/react-query';
...
export const HistoryTable = (): JSX.Element => {
  ...
  // ! API 호출
  const { response: HistoryFilters } = useHistoryFiltersEvidence({
    project_id: projectId,
  });
  // API 직접 호출
  const [actionFilters, setActionFilters] = useState<TListHistoryActionFilterData[]>([]);
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        const response = await fetchListHistoryActionFilter(); // API 호출
        if (response?.data) {
          setActionFilters(response.data); // ✅ 변환 없이 그대로 저장
        }
      } catch (error) {
        console.error('히스토리 필터 데이터를 가져오는 중 오류 발생:', error);
      }
    };
  ...
  const filters = {
    user_id: selectedUserIds.length > 0 ? selectedUserIds : [''],
    ...(selectedHistoryTypes.length > 0 && { type: selectedHistoryTypes }),
    ...(selectedPeriod &&
      selectedPeriod !== 'all' && {
        date_range: calculateStartDate(selectedPeriod),
      }),
  };

  const { response: HistoryList, refetch } = useFindHistoryList({
    project_id: projectId || '',
    page_no: selectedPage, // 현재 페이지
    block_cnt: itemsPerPage, // 페이지당 아이템 수
    start_date: selectedPeriod === 'all' ? '' : calculateStartDate(selectedPeriod).from,
    end_date: selectedPeriod === 'all' ? '' : calculateStartDate(selectedPeriod).to,
  ...
  const handlePageMove = (page = selectedPage) => {
    const totalPages = HistoryList?.pagination?.total_pages ?? 1;
  ...
  // 필터링된 히스토리 데이터 계산
  const filteredHistories = HistoryList?.histories.filter((history) => {
    if (!history) return false;

    const typeFilter = selectedHistoryTypes.length === 0 || selectedHistoryTypes.includes(history.type);

    const searchFilter =
      !searchKeyword ||
      history.content.toLowerCase().includes(searchKeyword.toLowerCase()) ||
      history.user_nm.toLowerCase().includes(searchKeyword.toLowerCase()) ||
      history.category.toLowerCase().includes(searchKeyword.toLowerCase()) ||
      history.type.toLowerCase().includes(searchKeyword.toLowerCase());
  ...
  return (
    <>
      <div className='flex justify-center pt-[220px]'>
        <div id='historyBody' className='w-[90%] lg:w-[90%] xl:w-[90%] 2xl:w-[90%]'>
          ...
          <div className='flex justify-center'>
            <div className='max-w-[320px ] h-full max-h-[calc(100vh-300px)] min-h-[calc(100vh-300px)] min-w-[290px] overflow-y-auto rounded-l-[8px] border-y border-l bg-white'>
              <div className='flex h-[52px] items-center border-b p-[16px]'>
                ...
                />
                <div
                  className={`ml-2 flex w-full justify-between ${getFontSizeClass(14, fontSizeAdjustment)}`}
                  style={{ fontSize: `${getAdjustedSize(14)}px` }}
                >
                  <div className='w-full text-[14px] text-[#252525]'>전체 {HistoryFilters?.length}명</div>
                      ...
                      <div className='ml-2 flex items-center'>
                        {filter.thumbnail ? (
                          <div className='relative h-[24px] w-[24px]'>
                            <img className='h-[24px] w-[24px] rounded-full' src={filter.thumbnail_url} alt='' />
                            <div
                        ...
                        <td className='w-[15%] whitespace-nowrap pl-[24px] text-sm font-medium text-gray-900'>
                          <div className='flex items-center'>
                            ...
                            history.thumbnail_url !== 'https://kr.object.ncloudstorage.com/ailex-bucket/' ? (
                              <div className='relative h-[24px] w-[24px]'>
                                <img src={history.thumbnail_url} alt='profile' className='h-full w-full rounded-full' />
                                <div
                                  className='absolute top-0 h-[24px] w-[24px] rounded-full border-2'
                                  style={{
