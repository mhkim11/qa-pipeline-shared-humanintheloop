...
import { useDownloadDocumentFile, useDragAndDropEvidence, useAddHistory } from '@query/mutation';
...
export const EvidenceTable = ({
  ...
  // URL 파라미터에서 officeId를 우선적으로 가져오고, 없으면 localStorage에서 가져옴
  const officeIdFromUrl = searchParams.get('officeId');
  const officeId = officeIdFromUrl || parsedData?.data?.user?.office_id;
  ...
  const handleViewDocument = async (evidenceId: string, pageCount: number) => {
    ...
    const url = `/admin/evidence-preview/pdf/${evidenceId}?projectId=${projectId}&officeId=${officeId}`;
  ...
  const handlePrint = () => {
    ...
    selectedItems.forEach(async (evidenceId) => {
      try {
        ...
        const printUrl = `/admin/evidence-preview/pdf/${evidenceId}?projectId=${projectId}&officeId=${officeId}&print=true`;
  ...
  const handleViewPDF = (pageCount: number) => {
    if (pageCount < 1) {
      onMessageToast({
        message: 'PDF문서가 없습니다.',
        icon: <IoIosWarning className='h-5 w-5 text-yellow-500' />,
    ...
    selectedItems.forEach((evidenceId) => {
      const newWindow = window.open(`/admin/evidence-preview/pdf/${evidenceId}?projectId=${projectId}&officeId=${officeId}`, '_blank');
      if (!newWindow) {
        onMessageToast({
  ...
  const handleViewText = () => {
    ...
    selectedItems.forEach((evidenceId) => {
      ...
      const newWindow = window.open(
        `/admin/evidence-preview/text/${evidenceId}?projectId=${projectId}&officeId=${officeId}&evidenceName=${encodedName}`,
        '_blank',
      );
      setTimeout(() => setSelectedItems([]), 500);
      if (!newWindow) {
  ...
  const handleSummaryPdfClick = (evidenceId: string) => {
    const url = `/admin/evidence-preview/summaryPdf/${evidenceId}?projectId=${projectId}&officeId=${officeId}`;
    window.open(url, '_blank');
  };
  ...
  }, [registerHandlers, selectedItems]);
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        <div className='relative'>
          ...
          <div
            id=''
            className={cn(
              `fixed left-[10px] top-[370px] z-[50] flex -translate-y-1/2 flex-col gap-3 rounded-[12px] bg-white p-[7px] shadow-lg`,
