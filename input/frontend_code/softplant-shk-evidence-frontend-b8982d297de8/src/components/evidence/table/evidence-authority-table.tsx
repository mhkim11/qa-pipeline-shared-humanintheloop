...
export const AuthorityTable = (): JSX.Element => {
  // ! api
  ...
  const [projectPaymentStatus, setProjectPaymentStatus] = useState<string | null>(null);
  const [projectExpireDate, setProjectExpireDate] = useState<string | null>(null);
  ...
  // 사건 정보 조회 (payment_status, expire_date 확인용)
  useEffect(() => {
    const fetchProjectInfo = async () => {
      if (projectId) {
        try {
          const projectData = await fetchFindCase(projectId);
          const paymentStatus = (projectData.data as any)?.payment_status || null;
          const expireDate = (projectData.data as any)?.expire_date || null;
          setProjectPaymentStatus(paymentStatus);
          setProjectExpireDate(expireDate);
        } catch (error) {
                  ...
                  onClick={async () => {
                    if (projectId) {
                      try {
                        ...
                        console.error('사건 조회 실패:', error);
                        setIsAuthModalOpen(true); // 에러가 나도 모달은 열기
                      }
                    } else {
                      setIsAuthModalOpen(true);
                    }
      ...
      {isAuthModalOpen && (
        <AuthModal
          ...
          onClose={() => {
            ...
            setProjectPaymentStatus(null);
            setProjectExpireDate(null);
          }}
          projectId={projectId || ''}
