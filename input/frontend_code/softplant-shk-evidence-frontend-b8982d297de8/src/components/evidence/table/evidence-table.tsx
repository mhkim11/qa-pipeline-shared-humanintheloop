...
// import { FiSearch, FiMaximize2, FiPlus } from 'react-icons/fi';
...
import { EvidenceOpinionDownloadModal } from '@/components/evidence/modal/evidence-opinion-download-modal';
import { EvidenceOpinionPopup } from '@/components/evidence/modal/evidence-opinion-popup';
import { EvidenceTagPopup } from '@/components/evidence/modal/evidence-tag-popup';
...
export const EvidenceTable = ({
  ...
  // ! 증거인부(의견) 관련 상태
  const [isEvidenceOpinionOpen, setIsEvidenceOpinionOpen] = useState(false);
  const [evidenceOpinionPosition, setEvidenceOpinionPosition] = useState({ top: 0, left: 0 });
  const [evidenceOpinionEvidenceId, setEvidenceOpinionEvidenceId] = useState<string | null>(null);
  const [evidenceOpinionIsAgreed, setEvidenceOpinionIsAgreed] = useState<boolean | null>(null);
  const [evidenceOpinionPages, setEvidenceOpinionPages] = useState<string>('');
  const [evidenceOpinionContent, setEvidenceOpinionContent] = useState<string>('');
  const opinionFormTouchedRef = useRef(false);

  // ! 증거인부 다운로드 관련 상태
  const [isOpinionDownloadOpen, setIsOpinionDownloadOpen] = useState(false);
  const [opinionDownloadMenuOpen, setOpinionDownloadMenuOpen] = useState(false);
  const [opinionDownloadTab, setOpinionDownloadTab] = useState<'ALL' | 'AGREE' | 'DISAGREE'>('ALL');
  ...
  const [selectedOpinionFilters, setSelectedOpinionFilters] = useState<string[]>([]);
  const [openFilter, setOpenFilter] = useState<string | null>(null);
  const [sortColumn, setSortColumn] = useState<string | undefined>(undefined);
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc' | undefined>(undefined);
  const opinionFilterOptions = useMemo(() => ['동의', '부동의', '의견없음'] as string[], []);
  ...
  // 사건 권한(사건관리자권한 여부) - "신규 증거 추가" 노출 제어용
  const { response: projectMembersResponse } = useFindProjectMembers({
    projectId: projectId || '',
    enabled: !!projectId,
  });
  const currentUserIsCaseManager = useMemo(() => {
    const userId = findEvidenceUserInfo?.data?.user_id;
    if (!userId) return false;
    const myRole = projectMembersResponse?.data?.members?.find((m) => m.user_id === userId)?.role;
    return myRole === '사건관리자권한' || myRole === '사건관리자';
  }, [findEvidenceUserInfo?.data?.user_id, projectMembersResponse?.data?.members]);
  ...
  // 증거인부(의견) 필터 처리: 서버에서 요구하는 값(동의/부동의/의견없음) 그대로 전달
  const processedOpinionFilters = useMemo(() => {
    if (!selectedOpinionFilters || selectedOpinionFilters.length === 0) return undefined;
    return selectedOpinionFilters;
  }, [selectedOpinionFilters]);
  ...
  const {
    ...
    filters: {
      ...
      opinion: processedOpinionFilters, // 증거인부 필터는 서버에서 처리(페이지네이션 반영)
    },
    sort_column: sortColumn,
    sort_direction: sortDirection,
  });

  const getOpinionSummary = useCallback((row: any) => {
    const opinions = (row?.opinions ?? []) as any[];
    const hasAgree = opinions.some((o) => o?.is_agreed === true);
    const hasDisagree = opinions.some((o) => o?.is_agreed === false);
    const hasMemo = opinions.some((o) => String(o?.content ?? '').trim().length > 0);
    return { hasAgree, hasDisagree, hasMemo, opinions };
  }, []);

  const getOpinionLabel = useCallback(
    (row: any) => {
      const { hasAgree, hasDisagree } = getOpinionSummary(row);
      if (hasAgree && hasDisagree) return '동의/부동의';
      if (hasAgree) return '동의';
      if (hasDisagree) return '부동의';
      return '';
    },
    [getOpinionSummary],
  );
  ...
  const handleHeaderSort = (column: string) => {
    ...
    // state 업데이트 후 API 호출 (증거인부 정렬은 로컬 처리)
    if (column === 'opinion') return;
    setTimeout(() => {
      refetchListEvidence();
    }, 0);
  };
  ...
  // ! 증거 목록 필터 리스트 api
  const { response: filterData, refetch: refetchFilterData } = useEvidenceFilter({
    project_id: projectId || '',
    keyword: '',
    filters: {
      key: [''],
  ...
  const { onRegisterEvidence } = useRegisterEvidence();
  const { onUpdateEvidence } = useUpdateEvidence();
  ...
  const currentUserId = parsedData?.data?.user?.id;

  // 증거인부(의견) 목록 조회: 수정 모드에서 최신 pages/content/is_agreed 를 보장하기 위해 사용
  const { response: listEvidenceOpinionOutput } = useListEvidenceOpinion({
    project_id: projectId || '',
    page_no: 1,
    block_cnt: 10000,
    enabled: !!projectId,
  ...
  // 증거인부는 "현재 사용자"에 종속되지 않고, evidence 자체의 opinion 데이터를 그대로 표시해야 함.
  // 동일한 evidence_id에 여러 opinion이 올 수 있으면 updatedAt 최신 1개를 채택.
  const opinionByEvidenceId = useMemo(() => {
    const map: Record<string, any> = {};
    const list = (listEvidenceOpinionOutput?.data?.list ?? []) as any[];
    list.forEach((o) => {
      const eviId = o?.evidence_id ? String(o.evidence_id) : '';
      if (!eviId) return;

      const prev = map[eviId];
      if (!prev) {
        map[eviId] = o;
        return;
      }
      const prevTime = prev?.updatedAt ? Date.parse(String(prev.updatedAt)) : 0;
      const nextTime = o?.updatedAt ? Date.parse(String(o.updatedAt)) : 0;
      if (nextTime >= prevTime) map[eviId] = o;
    });
    return map;
  }, [listEvidenceOpinionOutput]);

  const getLatestOpinionFromRow = useCallback(
    (row: any) => {
      const eviId = row?.evidence_id ? String(row.evidence_id) : '';
      if (!eviId) return undefined;
      const fromList = opinionByEvidenceId?.[eviId];
      if (fromList) return fromList;

      const opinions = (row?.opinions ?? []) as any[];
      if (!Array.isArray(opinions) || opinions.length === 0) return undefined;

      // updatedAt 최신 1개
      return opinions.reduce((latest, cur) => {
        const lt = latest?.updatedAt ? Date.parse(String(latest.updatedAt)) : 0;
        const ct = cur?.updatedAt ? Date.parse(String(cur.updatedAt)) : 0;
  ...
  const displayData = useMemo(() => {
    const base = listEvidenceOutput?.data?.results || [];

    // 증거인부(의견) 필터는 서버에서 처리 (페이지네이션 반영)
    const filtered = base;

    // 증거인부(의견) 정렬도 로컬에서 처리
    if (sortColumn === 'opinion' && (sortDirection === 'asc' || sortDirection === 'desc')) {
      const dir = sortDirection === 'asc' ? 1 : -1;
      return [...filtered].sort((a: any, b: any) => {
        const la = getOpinionLabel(a);
        const lb = getOpinionLabel(b);
        return dir * la.localeCompare(lb, 'ko');
      });
    }

    return filtered;
  }, [listEvidenceOutput, sortColumn, sortDirection, getOpinionLabel]);
  ...
  const handleRowClick = (evidenceId: string, index: number, event: React.MouseEvent) => {
    ...
    // 증거인부 셀(td)을 클릭한 경우 (체크박스 선택 방지)
    if ((event.target as HTMLElement).closest('td')?.classList.contains('opinion-cell')) {
      event.stopPropagation();
      return;
    }
  ...
  // 증거인부(의견) - content(의견) 존재 여부 (사용자 무관)
  const hasOpinionMemo = useCallback(
    (row: any) => {
      const fromList = row?.evidence_id ? opinionByEvidenceId?.[String(row.evidence_id)] : undefined;
      if (fromList) return String(fromList?.content ?? '').trim().length > 0;

      // fallback: list endpoint에 opinions가 포함된 경우 사용
      const opinions = (row?.opinions ?? []) as any[];
      return opinions.some((o) => String(o?.content ?? '').trim().length > 0);
    },
    [opinionByEvidenceId],
  );

  // 메모가 본인의 것인지 확인하는 함수
  const isMyMemo = (memoUserId: string) => {
  ...
  const handleViewDocument = async (evidenceId: string, pageCount: number) => {
    ...
    try {
      ...
      if (isMobile) {
        ...
        if (response.success && response.data?.url) {
          const googleViewerUrl = `https://docs.google.com/viewer?url=${encodeURIComponent(response.data.url)}&embedded=true`;
          window.open(googleViewerUrl, '_blank');
        } else {
          onMessageToast({
            message: 'PDF 문서를 불러올 수 없습니다.',
            icon: <IoIosWarning className='h-5 w-5 text-yellow-500' />,
          });
        }
      } else {
        // PC: 기존 뷰어 페이지 사용
        const viewerUrl = `/evidence/pdf/${evidenceId}?projectId=${projectId}&officeId=${officeId}`;
  ...
  // 증거인부 다운로드 드롭다운: 바깥 클릭 시 닫기
  useEffect(() => {
    if (!opinionDownloadMenuOpen) return;
    const onDown = (e: MouseEvent) => {
      if (!(e.target as Element).closest('[data-opinion-download-anchor="true"]')) {
        setOpinionDownloadMenuOpen(false);
      }
    };
    document.addEventListener('mousedown', onDown);
    return () => document.removeEventListener('mousedown', onDown);
  }, [opinionDownloadMenuOpen]);
  ...
  const handleSummaryPdfClick = (evidenceId: string) => {
    const url = `/evidence/summaryPdf/${evidenceId}?projectId=${projectId}&officeId=${officeId}`;
  ...
  const activeEvidenceOpinion = useMemo(() => {
    if (!evidenceOpinionEvidenceId) return undefined;
    const row = (listEvidenceOutput?.data?.results ?? []).find(
      (r: any) => String(r?.evidence_id ?? '') === String(evidenceOpinionEvidenceId),
    );
    if (row) return getLatestOpinionFromRow(row);
    return opinionByEvidenceId?.[String(evidenceOpinionEvidenceId)];
  }, [evidenceOpinionEvidenceId, listEvidenceOutput, getLatestOpinionFromRow, opinionByEvidenceId]);
  const isEvidenceOpinionEditMode = !!activeEvidenceOpinion;

  // 팝업을 먼저 열고 나중에 /opinion/list 가 도착하는 경우, 빈 값이면 최신 데이터로 1회 채움
  useEffect(() => {
    if (!isEvidenceOpinionOpen) return;
    if (!evidenceOpinionEvidenceId) return;
    if (!activeEvidenceOpinion) return;
    // 사용자가 내용을 수정/삭제한 뒤에는 자동 채움이 원복시키지 않도록 막음
    if (opinionFormTouchedRef.current) return;

    const nextPages = activeEvidenceOpinion?.pages != null ? String(activeEvidenceOpinion.pages) : '';
    const nextContent = activeEvidenceOpinion?.content ?? '';

    if (!String(evidenceOpinionPages ?? '').trim() && nextPages) setEvidenceOpinionPages(nextPages);
    if (!String(evidenceOpinionContent ?? '').trim() && String(nextContent).trim()) setEvidenceOpinionContent(nextContent);
    if (evidenceOpinionIsAgreed == null && typeof activeEvidenceOpinion?.is_agreed === 'boolean') {
      setEvidenceOpinionIsAgreed(!!activeEvidenceOpinion.is_agreed);
    }
  }, [
    isEvidenceOpinionOpen,
    evidenceOpinionEvidenceId,
    activeEvidenceOpinion,
    evidenceOpinionPages,
    evidenceOpinionContent,
    evidenceOpinionIsAgreed,
  ]);

  return (
    <S.AllBody>
      <EvidenceOpinionPopup
        isOpen={!!(isEvidenceOpinionOpen && evidenceOpinionEvidenceId)}
        position={evidenceOpinionPosition}
        isEditMode={isEvidenceOpinionEditMode}
        isAgreed={evidenceOpinionIsAgreed}
        pages={evidenceOpinionPages}
        content={evidenceOpinionContent}
        onChangeIsAgreed={(v) => {
          opinionFormTouchedRef.current = true;
          setEvidenceOpinionIsAgreed(v);
        }}
        onChangePages={(v) => {
          opinionFormTouchedRef.current = true;
          setEvidenceOpinionPages(v);
        }}
        onChangeContent={(v) => {
          opinionFormTouchedRef.current = true;
          setEvidenceOpinionContent(v);
        }}
        onClose={() => {
          setIsEvidenceOpinionOpen(false);
          setEvidenceOpinionEvidenceId(null);
          opinionFormTouchedRef.current = false;
        }}
        onApply={async () => {
          if (!projectId) return;
          if (!evidenceOpinionEvidenceId) return;
          if (evidenceOpinionIsAgreed == null) return;

          const pagesStr = String(evidenceOpinionPages || '').trim();
          if (!pagesStr) {
            onMessageToast({
              message: '쪽수를 입력해주세요.',
              icon: <IoIosWarning className='h-5 w-5 text-yellow-500' />,
          ...
          const res = isEvidenceOpinionEditMode
            ? await (async () => {
                const opinionId = String(activeEvidenceOpinion?.opinion_id ?? '').trim();
                if (!opinionId) {
                  onMessageToast({
                    message: 'opinion_id가 없어 수정할 수 없습니다. 새로고침 후 다시 시도해주세요.',
                    icon: <IoIosWarning className='h-5 w-5 text-yellow-500' />,
                  });
                  return undefined;
                }
                return await onUpdateEvidence({
                  project_id: projectId,
                  opinion_id: opinionId,
                  is_agreed: evidenceOpinionIsAgreed,
                  pages: pagesStr,
                  content: evidenceOpinionContent,
                });
              })()
            : await onRegisterEvidence({
                project_id: projectId,
                evidence_id: evidenceOpinionEvidenceId,
                is_agreed: evidenceOpinionIsAgreed,
                pages: pagesStr,
                content: evidenceOpinionContent,
              });
          ...
          if (res?.isSuccess) {
            onMessageToast({
              message: isEvidenceOpinionEditMode ? '증거인부가 수정되었습니다.' : '증거인부가 등록되었습니다.',
              icon: <IoMdSend className='h-5 w-5 text-green-500' />,
            });
            refetchListEvidence();
            setIsEvidenceOpinionOpen(false);
            ...
            onMessageToast({
              message: res?.message || '요청에 실패했습니다.',
              icon: <IoIosWarning className='h-5 w-5 text-yellow-500' />,
            });
          }
        }}
          ...
          <div className='hidden lg:flex'>
            <div className='relative'>
              ...
              {searchExecuted && !powerSearch && tempInput && (
                <>
                  <button className='mr-[16px] h-[48px] w-[105px] rounded-[8px] bg-[#004AA4] text-[16px] text-white' onClick={handleReset}>
                    검색 초기화
                  </button>
                </>
              )}
                    ...
                    >
                      <thead className='evidence-table-header sticky top-0 z-50 bg-[#E6EFFF] font-bold text-[#252525]'>
                        ...
                        <tr className='h-[52px]'>
                          ...
                          <th
                            className='w-[10%] min-w-[120px] cursor-col-resize whitespace-nowrap pb-[26px] pt-[8px] hover:after:absolute hover:after:right-0 hover:after:top-0 hover:after:z-10 hover:after:mr-[1.5px] hover:after:h-full hover:after:w-[6px] hover:after:bg-[#97C0F1] hover:after:content-[""]'
                            data-column-id='opinion'
                            style={{
                              width: `${columnWidths.opinion}px`,
                              minWidth: `${columnWidths.opinion}px`,
                              position: 'relative',
                            }}
                            onMouseDown={(e) => {
                              if ((e.target as HTMLElement).closest('button')) {
                          ...
                          >
                            <div className='flex items-center justify-center'>
                              ...
                              <EvidenceListFilter
                                column='증거인부'
                                options={opinionFilterOptions}
                                onFilter={(values) => {
                                  setSelectedOpinionFilters(values);
                                  // 서버 필터로 처리해야 페이지네이션(total/pages) 반영됨
                                  setCurrentPage(1);
                                  setSelectedPage(1);
                                  if (currentPage === 1) {
                                    setTimeout(() => {
                                ...
                                isOpen={openFilter === 'opinion'}
                                onToggle={() => handleFilterToggle('opinion')}
                              />
                            </div>
                          </th>
                      ...
                      <tbody {...provided.droppableProps} ref={provided.innerRef} className='overflow-y-auto'>
                        {isProcessing || isLoading || isSearching ? (
                          <tr className=''>
                            <td colSpan={16} className='h-[300px] text-center' style={{ minWidth: `${totalTableMinWidth}px` }}>
                              <CustomSpinner />
                            </td>
                          </tr>
                                    ...
                                    >
                                      ...
                                      {tagWritingId === item.evidence_id && isTagPopupOpen && (
                                        <EvidenceTagPopup
                                          ...
                                          existingTags={item.tags?.map((tag: any) => ({
                                            tag_set_id: tag.tag_set_id || tag.tag_id,
                                            tag_id: tag.tag_id, // tag_id도 함께 전달
                                            tag_name: tag.tag_name,
                                            color: tag.color,
                                          }))}
                                          onTagUpdate={() => {
                                            refetchListEvidence();
                                            refetchFilterData(); // 태그 생성/수정/삭제 시 필터 목록도 리패치
                                          }}
                                        />
                                      )}
                                      ...
                                      <td
                                        ...
                                        onClick={(e) => {
                                          ...
                                          const existing = getLatestOpinionFromRow(item);
                                          const start = Number(item?.start_page ?? 0);
                                          const pageCount = Number(item?.page_count ?? 0);
                                          const end = start && pageCount ? start + pageCount - 1 : start || 0;
                                          const defaultPages = start ? `${start}-${end || start}` : '';
                                          setEvidenceOpinionEvidenceId(item.evidence_id);
                                          setEvidenceOpinionIsAgreed(existing ? !!existing.is_agreed : true);
                                          setEvidenceOpinionPages(existing?.pages != null ? String(existing.pages) : defaultPages);
                                          setEvidenceOpinionContent(existing?.content ?? '');
                                          opinionFormTouchedRef.current = false;
                                          setEvidenceOpinionPosition({ top, left });
                                          setIsEvidenceOpinionOpen(true);
                                        }}
                                      >
                                        <div className='flex items-center justify-center gap-1'>
                                          {hasOpinionMemo(item) ? <IoDocumentOutline className='text-[16px] text-[#252525]' /> : null}
                                          <span className={cn('text-[12px]', getOpinionLabel(item) ? 'text-[#212121]' : 'text-[#A1A1AA]')}>
                                            {(() => {
                                              const { hasAgree, hasDisagree } = getOpinionSummary(item);
                                              if (hasAgree && hasDisagree) return '동의 부동의';
                                              if (hasAgree) return '동의';
                                              if (hasDisagree) return '부동의';
                                              return '-';
                          ...
                          // 검색 결과 없음
                          <tr>
                            <td colSpan={16} className='pb-6 pt-8 text-center text-gray-500'>
                              검색 결과가 없습니다.
                            </td>
                          </tr>
                        )}
      ...
      <div className='fixed bottom-0 z-[50] flex h-[78px] w-full items-center justify-center bg-white shadow-inner'>
        <div id='footerBody' className='flex items-center justify-between'>
          ...
          <div className='ml-[18px]'>
            <div className='relative hidden items-center gap-2 lg:flex' data-opinion-download-anchor='true'>
              <div
                onClick={downloadExcel}
                className='h-[32px] w-[101px] cursor-pointer items-center justify-center rounded-[8px] border text-[14px] text-[#252525] lg:flex'
              >
                엑셀 다운로드
              </div>
              ...
              <button
                type='button'
                className='h-[32px] w-[130px] rounded-[8px] border text-[14px] text-[#252525] hover:bg-[#F4F4F5]'
                onClick={() => setOpinionDownloadMenuOpen((v) => !v)}
              >
                증거인부 다운로드
              </button>

              {opinionDownloadMenuOpen && (
                <div className='absolute bottom-[40px] right-0 z-[9999] w-[120px] overflow-hidden rounded-[8px] border border-[#E5E5E5] bg-white shadow-lg'>
                  {[
                    { key: 'ALL' as const, label: '전체' },
                    { key: 'AGREE' as const, label: '동의' },
                    { key: 'DISAGREE' as const, label: '부동의' },
                  ].map((opt) => (
                    <button
                      key={opt.key}
                      type='button'
                      className='flex h-[32px] w-full items-center px-3 text-[14px] text-[#252525] hover:bg-[#F4F4F5]'
                      onClick={() => {
                        setOpinionDownloadMenuOpen(false);
                        setOpinionDownloadTab(opt.key);
                        setIsOpinionDownloadOpen(true);
                      }}
                    >
                      {opt.label}
                    </button>
      ...
      */}
      ...
      <EvidenceOpinionDownloadModal
        isOpen={isOpinionDownloadOpen}
        initialTab={opinionDownloadTab}
        projectId={projectId || ''}
        onClose={() => setIsOpinionDownloadOpen(false)}
      />
    </S.AllBody>
  );
};
