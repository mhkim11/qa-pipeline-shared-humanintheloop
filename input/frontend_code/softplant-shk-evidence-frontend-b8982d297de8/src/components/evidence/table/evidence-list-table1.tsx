...
import { CTALine, MainHeader } from '@components/common';
...
import { useFindAllEvidenceList, useFindUserInfo } from '@/hooks/react-query';
import { useCreateProjectFilter } from '@/hooks/react-query/mutation';
...
export const EvidenceListTable = (): JSX.Element => {
  ...
  const [assignedMe, setAssignedMe] = useState(() => {
    const savedAssignedMe = localStorage.getItem('assignedMe');
    return savedAssignedMe !== null ? JSON.parse(savedAssignedMe) : false;
  });
  ...
  const handleAssignedMeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setAssignedMe(e.target.checked);
    // 상태 변경 후 즉시 refetch 수행
    setTimeout(() => {
      refetch();
    }, 0);
  };
  ...
  const filteredEvidenceList = useMemo(() => {
    ...
    return evidenceList.filter((item) => {
      // 종결 상태 확인
      const statusMatch = isFinished ? item.status === '종결' : item.status !== '종결';
  ...
  const updateFilters = () => {
    const filteredOptions: TFilterOptions = {
      status: Array.from(new Set(evidenceList.map((item) => item.status))),
    ...
    if (openDropdown === key) {
      ...
      try {
        const response = await onCreateProjectFilter({
          ...
          assignedMe: assignedMe, // assignedMe 상태 포함
        });
  ...
  // ! 내사건만 보기 체크
  useEffect(() => {
    localStorage.setItem('assignedMe', JSON.stringify(assignedMe));
  }, [assignedMe]);
  useEffect(() => {
    const handleActivityAndUpdates = () => {
      const savedAssignedMeFromStorage = localStorage.getItem('assignedMe');
      if (savedAssignedMeFromStorage !== null) {
        const newAssignedMeValue = JSON.parse(savedAssignedMeFromStorage);

        setAssignedMe((currentAssignedMe: boolean) => {
          if (currentAssignedMe !== newAssignedMeValue) {
            return newAssignedMeValue;
          }
          return currentAssignedMe;
        });
      }
    ...
    window.addEventListener('focus', handleActivityAndUpdates);

    handleActivityAndUpdates();

    return () => {
      window.removeEventListener('focus', handleActivityAndUpdates);
    };
  }, [refetch]);
  ...
  return (
    <div className='flex h-screen flex-col overflow-hidden'>
      ...
      <MainHeader />
      {/* 검색 */}
      <div className='mt-[100px] flex h-[calc(100vh-64px)] justify-center overflow-hidden'>
        <div id='evidence-table-body' className='flex h-full w-[85%] flex-col lg:w-[85%] xl:w-[85%] 2xl:w-[70%]'>
          <div className=''>
            <div className=''>
              <div className='flex h-[104px] w-full items-center justify-between'>
                <div className='flex'>
                  ...
                  <div className='ml-[24px] flex items-center'>
                    <input
                      type='checkbox'
                      className='h-[20px] w-[20px] rounded-[2px] border-[1.5px] border-[#BEC1C6] text-[#0050B3] focus:ring-[#4577A4]'
                      checked={assignedMe}
                      onChange={handleAssignedMeChange}
                      id='assignedMe'
                    />

                    <label
                      htmlFor='assignedMe'
                      className={`ml-[12px] mr-[16px] text-[14px] text-[#666] ${getFontSizeClass(14, fontSizeAdjustment)}`}
                            ...
                            <table className='min-w-full'>
                              <thead className='sticky top-0 z-20 bg-[#E6EFFF] font-bold text-[#272727] lg:text-[14px]'>
                                <tr className=''>
                                  ...
                                  <th scope='col' className='w-[10%] min-w-[100px] whitespace-nowrap py-3'>
                                    <div className='flex items-center'>
                                      <div className='mr-1 flex text-[#999]'>
                                  ...
                                  <th scope='col' className='w-[10%] min-w-[100px] whitespace-nowrap rounded-tr-[8px] py-3'>
                                    <div className='flex items-center'>
                                      <div className='mr-1 flex text-[#999]'>
                              ...
                              {!hasLoadedData ? (
                                <tbody className='h-[400px] text-center'>
                                  <tr>
                                    <td colSpan={8} className='pb-6 pt-8 text-gray-500'>
                                      <CustomSpinner />
                                    </td>
                                  </tr>
                                </tbody>
                                  ...
                                  {filteredEvidenceList.map((evidence, index) => (
                                    ...
                                    >
                                      ...
                                      <td className='w-[8%] text-center'>
                                        <div className='flex items-center'>
                                          {evidence.status === '진행중' ? (
                                            <div className='flex h-[28px] w-[60px] items-center justify-center rounded-full bg-[#E6F7FF] text-[14px] text-[#1890FF]'>
                                              진행 중
                                            </div>
                                          ) : evidence.status === '종결' ? (
                                            ...
                                            <div className='flex h-[28px] w-[60px] items-center justify-center rounded-full bg-[#FFE58F] text-[14px] text-[#AD6800]'>
                                              생성 중
                                            </div>
                                          )}
                                        </div>
                                      ...
                                      <td
                                        className={`w-[10%] truncate pl-2 text-[#888] ${getFontSizeClass(14, fontSizeAdjustment)}`}
                                        style={{ fontSize: `${getAdjustedSize(14)}px` }}
                                      >
                                        {evidence.client_nm}
                                      </td>
                                      ...
                                      <td
                                        className={`w-[10%] pl-2 text-[#888] ${getFontSizeClass(14, fontSizeAdjustment)}`}
                                        style={{ fontSize: `${getAdjustedSize(14)}px` }}
                                      >
                                        {' '}
      ...
      <div className='fixed bottom-0 z-50 flex h-[78px] w-full items-center justify-center bg-white shadow-inner'>
        <div id='evidence-table-body' className='flex w-[90%] items-center justify-between 2xl:w-[70%]'>
          <div
            className={`flex min-w-[100px] items-center font-light text-[#5b5b5b] 2xl:min-w-[200px] ${getFontSizeClass(12, fontSizeAdjustment)}`}
            style={{ fontSize: `${getAdjustedSize(12)}px` }}
          >
            전체 사건{' '}
          ...
          <div className='flex items-center justify-center'>
            ...
            />
            <div className='ml-[5px] flex items-center justify-center 2xl:ml-[26px]'>
              <input
                type='text'
                value={selectedPage}
                onChange={(e) => {
          ...
          <div className=''>
            <button
              type='button'
              className='flex h-[48px] w-[150px] items-center justify-center rounded-[10px] border border-[#0050B3] text-[14px] text-[#0050B3]'
              onClick={() => setIsUploadModalOpen(true)}
            >
