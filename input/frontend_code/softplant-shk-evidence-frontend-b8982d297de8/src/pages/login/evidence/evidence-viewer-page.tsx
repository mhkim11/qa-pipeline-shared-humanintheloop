...
import { useViewDocument, useDownloadDocumentFile } from '@query/mutation';
...
const EvidenceViewer = () => {
  ...
  const fetchPdf = useCallback(async () => {
    ...
    try {
      ...
      // 모바일인 경우 서버 URL 직접 사용 또는 Google Docs Viewer 사용
      if (isMobile) {
        try {
          console.log('모바일 감지됨, 서버 URL 가져오기 시도');
          const response = await onDownloadDocumentFile(input);
          console.log('모바일 응답:', response);
          // response.data가 배열인 경우 첫 번째 요소 사용, 객체인 경우 직접 사용
          const downloadData = Array.isArray(response.data) ? response.data[0] : response.data;
          console.log('다운로드 데이터:', downloadData);
          if (response.success && downloadData?.url) {
            // 갤럭시 탭 등에서 서버 URL 직접 사용
            pdfUrlRef.current = downloadData.url;
            console.log('모바일 서버 URL 설정:', downloadData.url);
            setLoading(false);
            setShowPdf(true);
            return;
          } else {
            console.warn('모바일 서버 URL 가져오기 실패:', response);
          }
        } catch (mobileError) {
          console.error('모바일 서버 URL 실패, blob URL로 폴백:', mobileError);
          // 서버 URL 실패 시 blob URL로 폴백
        }
      }

      // PC 또는 모바일 폴백: blob URL 사용
  ...
  }, [evidenceId, projectId, officeId, isMobile, onViewDocument, onDownloadDocumentFile]);
  useEffect(() => {
    fetchPdf();
