...
4. **viewport 1280×800** 고정, full page 기준으로 캡처
5. **캡처 포맷: PNG** — 파일명은 `pipeline_config.md` 캡처 대상 화면의 파일명 그대로 사용
6. 캡처 대상 URL에 `{변수명}` 형태가 있으면 동일 파일의 **테스트 데이터 ID** 섹션에서 값을 찾아 치환
   - 예: `{CIVIL_CASE_ID}` → `CIVIL_CASE_ID = ccase_xxx` 값으로 대체
   - 대응하는 ID가 없으면 해당 화면은 건너뛰고 `capture_errors.log`에 기록
7. **탭이 여러 개인 화면**은 탭별로 별도 캡처
   - 탭 클릭 후 네트워크 idle 대기 → 캡처
   - 파일명: `pipeline_config.md`의 파일명 기준 (탭별로 이미 분리 명시된 경우 해당 파일명 사용)

---

## AI 판단 규칙

1. 로그인 실패 시 → **즉시 중단** — 이후 모든 화면 캡처 불가. `capture_errors.log`에 실패 기록 후 종료
2. 팝업/모달 발생 시 → 닫고 계속 진행
3. 로딩 10초 이상 → 타임아웃 기록 후 다음 화면 진행
4. 로그인 세션 만료 시 → 재로그인 후 이어서 진행
5. 캡처 실패 시 → `capture_errors.log`에 사유 기록
...
- 화면별 캡처 이미지 (`pipeline_config.md` 캡처 대상 화면 수와 동일, PNG, 1280×800 viewport)
- `capture_errors.log` — 아래 형식 고정:
  ```
  [YYYY-MM-DD HH:MM:SS] OK   파일명.png
  [YYYY-MM-DD HH:MM:SS] FAIL 파일명.png — 실패 사유
  [YYYY-MM-DD HH:MM:SS] INFO 메시지 (로그인 성공, 캡처 완료 요약 등)
  ```
- `capture.py`
