# Step 1 — Claude Code (시나리오 생성) / 전자동
...
1. `~/qa-pipeline/input/` 의 프론트엔드 코드 zip 압축 해제 후 라우팅 구조 분석
   - 화면 목록, 화면 간 이동 경로, 주요 기능 추출

2. Figma API로 파일 프레임 목록과 Annotation 추출
   - 추출한 프레임 이미지는 `~/qa-pipeline/input/figma_frames/` 에 저장
...
```bash
git -C ~/qa-pipeline add input/figma_frames/ output/step1/
git -C ~/qa-pipeline commit -m "step1: 시나리오 및 정의서 생성"
...
> 다음 단계: **Step 2** → `instructions/step2_manus_전자동.md` 참고
