# Step 3 — Claude Code + Playwright (화면 캡처) / 전자동
...
```bash
git -C ~/qa-pipeline pull origin main
```
...
모든 인증 정보는 `~/qa-pipeline/.env` 파일 참고
...
```
~/qa-pipeline/scripts/capture.py
```
...
```
~/qa-pipeline/output/step3/screenshots/
~/qa-pipeline/output/step3/capture_errors.log
~/qa-pipeline/scripts/capture.py
```
...
```bash
git -C ~/qa-pipeline add output/step3/ scripts/capture.py
git -C ~/qa-pipeline commit -m "step3: Playwright 화면 캡처"
git -C ~/qa-pipeline push origin main
```
...
> 다음 단계: **Step 4** → `instructions/step4_claude_code_전자동.md` 참고
