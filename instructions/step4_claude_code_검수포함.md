...
6. `qa_results.csv`에 `Step4포함` 컬럼이 있고 값이 `N`인 행 → `qa_final_report.csv` 및 HTML 리포트에서 제외
7. `extra_bugs.md`가 있는 경우 → HTML 리포트에 **시나리오 외 추가 버그** 섹션으로 별도 포함
...
- 시나리오 외 추가 버그 섹션 (`extra_bugs.md` 있는 경우)
- 개선 권고사항 섹션
